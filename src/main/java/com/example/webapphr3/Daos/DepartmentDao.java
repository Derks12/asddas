...
public class DepartmentDao extends DaoBase {
    ...
    public BigDecimal obtenerSalarioMaximoPorDepartamento(String nombreDepartamento) {
        BigDecimal salarioMaximo = BigDecimal.ZERO;

        String sql = "SELECT MAX(salary) FROM employees WHERE department_id = " +
        ...
        try (Connection conn = this.getConection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    salarioMaximo = rs.getBigDecimal(1);
                }
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
